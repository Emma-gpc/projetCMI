...
function traduction(listeBlocs){//prend en paramêtre la liste des blocs
    var expression=''; var entrees; //expression contiendra l'expréssion final et entrees est une pile qui va contenir les entrées d'un blocs
    for(e in listeBlocs){//parcour des différents blocs
        for(let i=0;i<listeBlocs[e].length;i++){//parcour des sommets présent dans le blocs e
            //traduction de la position du sommet en cour d'analise en entrées corsepondantes
            if(e[i][0]=='0'){//ce if détermine si le sommet est sur la première ligne
                if(e[i][1]=='0'){//ce if détermine si le sommet est sur la première colonne
                    entrees.empiler('!A.!B.!C.!D');
                }
                else if(e[i][1]=='1'){//ce if détermine si le sommet est sur la deuxième colonne
                    entrees.empiler('!A.!B.!C.D');
                }
                else if(e[i][1]=='2'){//ce if détermine si le sommet est sur la troisième colonne
                    entrees.empiler('!A.!B.C.D');
                }
                else if(e[i][1]=='3'){//ce if détermine si le sommet est sur la quatrième colonne
                    entrees.empiler('!A.!B.C.!D');
                }
            }
            else if(e[i][0]=='1'){//ce if détermine si le sommet est sur la deuxième ligne
                if(e[i][1]=='0'){//ce if détermine si le sommet est sur la première colonne
                    entrees.empiler('!A.B.!C.!D');
                }
                else if(e[i][1]=='1'){//ce if détermine si le sommet est sur la deuxième colonne
            ...
            else if(e[i][0]=='2'){//ce if détermine si le sommet est sur la troisième ligne
                if(e[i][1]=='0'){//ce if détermine si le sommet est sur la première colonne
                    entrees.empiler('A.B.!C.!D');
                }
                else if(e[i][1]=='1'){//ce if détermine si le sommet est sur la deuxième colonne
            ...
            else if(e[i][0]=='3'){//ce if détermine si le sommet est sur la quatrième ligne
                if(e[i][1]=='0'){//ce if détermine si le sommet est sur la première colonne
                    entrees.empiler('A.!B.!C.!D');
                }
                else if(e[i][1]=='1'){//ce if détermine si le sommet est sur la deuxième colonne
        ...
        for(let j=1;j<entrees.length;j++){//parcour de la pile d'entrées à partir du deuxième élèment
            for(let k=0;k<j.length;k++){//parcour de la chaine de caractère j
                if(entrees[0][k]!=entrees[j][k]){//on cherche une difference entre les groupes d'entrées
                    var dif='';//variable de sauvegarde
                    if(entrees[0][k]!='!'){//si le if est vrai,cela veux dire qu'il faut utiliser l'entree de entrees[0]
                        dif=entrees[j];//on sauvegarde l'entree qui va être modifié
                        for(let l=0;l<=k;l++){//on parcour l'entree à modifier jusqu'à la difference
                            entrees[j][l]=entrees[0][l];//on recopie le début de entrees[0] jusqu'à la difference sur entrees[j]
                        }
                        for(let m=k+2;entrees[j].length;m++){//on parcour entrees[j] juste après la difference jusqu'à la fin
                            entrees[j][m-1]=dif[m];//on recopie la fin de entrees[j] pour à la fin avoir la même entree sans le ! en trop
                        }
                    }
                    else{//même chose mais en inversant entrees[0] et entrees[j]
                        dif=entrees[0];
                        for(let l=0;l<=k;l++){
                            entrees[0][l]=entrees[j][l];
                        }
                        for(let m=k+2;entrees[0].length;m++){
                            entrees[0][m-1]=dif[m];
                        }
                    }
                }
        ...
        //à la fin de la boucle on obtion l'expréssion simplifiée du bloc
        if(expression.length>0){//si expression n'est pas vide on ajoute l'entree du bloc qui à été simplifié
            expression=expression+'+'+entrees[0];
        }
        else{// sinon expression devien l'entree simplifié du bloc
            expression=entrees[0];
        }
    }
}
